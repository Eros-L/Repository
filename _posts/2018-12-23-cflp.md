...
通过比较 Greedy 和 Tabu 的结果，我们能发现 Tabu 在每个实例上都能取得比 Greedy 好的数据，且时间开销平均只为 Greedy 的十倍（每个实例不超过 0.5s）。在某些实例上，Tabu 得到方案的开销仅为 Greedy 的一半左右，则证明使用 Tabu 要远优于使用 Greedy。最后，项目的全部文件保存在[以下文件夹中](https://github.com/Eros-L/Eros-L.github.io/tree/master/_posts/cflp)，更详细的结果可在 github 上查看。 <br />
